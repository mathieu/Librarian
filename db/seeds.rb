...
if Comic.where(isbn:'9782723463409').blank?
  comic = Comic.create( title:'Les Gouttes de Dieu 1', isbn: '9782723463409',
                registered:'2012/05/13', available: true, collection_id: Collection.find_by_title('Les Gouttes de Dieu').id)
  comic.authors << Author.where(name: 'Tadashi AGI')
  comic.authors << Author.where(name: 'Shu OKIMOTO')
  comic.author_names = 'Tadashi AGI, Shu OKIMOTO'
  comic.save
...
if Comic.where(isbn:'9784063724356').blank?
  comic = Comic.create( title:'Les Gouttes de Dieu 2', isbn: '9784063724356',
                registered:'2012/05/13', available: true, collection_id: Collection.find_by_title('Les Gouttes de Dieu').id)
  comic.authors << Author.where(name: 'Tadashi AGI')
  comic.authors << Author.where(name: 'Shu OKIMOTO')
  comic.author_names= 'Tadashi AGI, Shu OKIMOTO'
...
if Comic.where(isbn:'9782858151257').blank?
  comic = Comic.create( title:'Soeur Marie-Thérèse des Batignoles', isbn: '9782858151257',
                registered:'2011/04/13', available: true)
  comic.authors << Author.where(name: 'Maëster')
  comic.author_names= 'Maëster'
  comic.save
